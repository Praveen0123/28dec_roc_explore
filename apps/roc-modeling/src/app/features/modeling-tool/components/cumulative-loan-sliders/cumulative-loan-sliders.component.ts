...
export class CumulativeLoanSlidersComponent implements OnInit, OnChanges
{
  ...
  setLocalVariables()
  {
    this.isTaxDependent = this.roiModel.educationFinancing.isTaxDependent;
    this.outOfPocketExpensesByYear = this.roiModel.educationFinancing.outOfPocketExpensesByYear;

    this.netPriceByYear = this.roiModel.netPriceByYear;
    this.cumulativeNetPrice = this.arraySum(this.roiModel.netPriceByYear);
    this.cumulativeOutOfPocketExpenses = this.arraySum(this.roiModel.outOfPocketExpensesByYear);
    this.cumulativeFederalSubsidizedLoanLimit = this.arraySum(this.roiModel.federalSubsidizedLoanLimitByYear);
    this.cumulativeFederalUnsubsidizedLoanLimit = this.arraySum(this.roiModel.federalUnsubsidizedLoanLimitByYear);
    this.yearsToCompleteDegree = this.roiModel.educationCost.yearsToCompleteDegree;
    this.costOfAttendanceByYear = this.roiModel.costOfAttendanceByYear;
    this.efc = this.roiModel.efc;
    this.fullTimeStudentPercent = this.roiModel.educationCost.isFulltime ? 1 : 0.5;
    this.grantOrScholarshipAidExcludingPellGrant = this.roiModel.grantOrScholarshipAidExcludingPellGrant;
